...
def cornersHeuristic(state, problem):
    ...
    if problem.isGoalState(state):
        ...
        for index,item in enumerate(state[1]):
            if item == 0:
                distancesFromGoals.append(util.manhattanDistance(state[0],corners[index]))
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def registerInitialState(self, state):
        ...
        while(current_state.getFood().count() > 0):
            next_PathSegment = self.findPathToClosestDot(current_state)
            self.actions += next_PathSegment
            for action in next_PathSegment:
