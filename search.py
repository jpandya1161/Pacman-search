...
def breadthFirstSearch(problem):
    ...
    marked = [False]*problem.getStartState().size()

    def bfs(problem.getStartState(), v):
        visited = []
        path = []
...
def uniformCostSearch(problem):
    ...
    while(True):
        ...
        if successors:
            for item in successors:
                if item[0] not in visited and (item[0] not in (state[2][0] for state in p_queue.heap)):
                    newPath = path + [item[1]]
                    price = problem.getCostOfActions(newPath)
                    p_queue.push((item[0],newPath),price)

                elif item[0] not in visited and (item[0] in (state[2][0] for state in p_queue.heap)):
                    for state in p_queue.heap:
                        if state[2][0] == item[0]:
                            oldPrice = problem.getCostOfActions(state[2][1])

                    newPrice = problem.getCostOfActions(path + [item[1]])

                    if oldPrice > newPrice:
                        newPath = path + [item[1]]
                        p_queue.update((item[0],newPath),newPrice)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    "*** YOUR CODE HERE ***"
    pqueue = util.PriorityQueuewithFunction(problem, func)
